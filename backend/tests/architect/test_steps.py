...
import pytest
from architect import steps
from architect.evaluator_reader import read_evaluator
...
def test_choose_orchestration_parses_mode_and_reason(evaluator):
    complete = FakeComplete(
        [json.dumps({"mode": "single", "reason": "One call suffices."})]
    )
    result, response = steps.choose_orchestration(
        "goal", "domain", evaluator, complete, "strong"
    )
...
def test_choose_orchestration_retries_once_on_bad_json_then_succeeds(evaluator):
    complete = FakeComplete(
        [
            "not json",
            json.dumps({"mode": "planner_worker", "reason": "Needs planning."}),
        ]
    )
    result, _ = steps.choose_orchestration(
        "goal", "domain", evaluator, complete, "strong"
    )
...
def test_choose_orchestration_raises_after_a_failed_retry(evaluator):
    complete = FakeComplete(["not json", "still not json"])
    with pytest.raises(steps.ArchitectStepError):
        steps.choose_orchestration("goal", "domain", evaluator, complete, "strong")


def test_choose_orchestration_rejects_an_unknown_mode(evaluator):
    complete = FakeComplete(
        [
            json.dumps({"mode": "yolo", "reason": "x"}),
            json.dumps({"mode": "yolo", "reason": "x"}),
        ]
    )
    with pytest.raises(steps.ArchitectStepError):
...
def test_draft_prompt_returns_the_raw_text_when_it_already_mentions_json_and_keys(
    ...
    complete = FakeComplete([text])
    orchestration = {"mode": "single", "reason": "r"}
    result, _ = steps.draft_prompt(
        "goal", "domain", evaluator, orchestration, complete, "strong"
    )
    assert result == text


def test_draft_prompt_appends_an_output_format_footer_when_missing(evaluator):
    complete = FakeComplete(["Be a good triage agent."])
    orchestration = {"mode": "single", "reason": "r"}
    result, _ = steps.draft_prompt(
        "goal", "domain", evaluator, orchestration, complete, "strong"
    )
...
def test_draft_prompt_rejects_empty_text(evaluator):
    complete = FakeComplete(["   "])
    orchestration = {"mode": "single", "reason": "r"}
    with pytest.raises(steps.ArchitectStepError):
        steps.draft_prompt(
...
def test_select_tools_returns_the_requested_subset(evaluator):
    complete = FakeComplete(
...
def test_select_tools_falls_back_to_the_full_allow_list_when_the_model_picks_nothing_usable(
    ...
    complete = FakeComplete([json.dumps({"tools": ["not_allowed"], "glue_tool": None})])
    result, _ = steps.select_tools(
        "goal", "domain", evaluator, ["json_validate", "date_parse"], complete, "strong"
    )
...
def test_select_tools_raises_when_the_allow_list_is_empty(evaluator):
    complete = FakeComplete([])
    with pytest.raises(steps.ArchitectStepError):
...
def test_select_tools_accepts_a_syntactically_valid_glue_tool(evaluator):
    complete = FakeComplete(
...
def test_select_tools_drops_a_glue_tool_with_invalid_python(evaluator):
    bad = {**VALID_GLUE_TOOL, "code": "def broken(:\n"}
    complete = FakeComplete(
        [json.dumps({"tools": ["json_validate"], "glue_tool": bad})]
    )
    result, _ = steps.select_tools(
        "goal", "domain", evaluator, ["json_validate"], complete, "strong"
    )
...
def test_select_tools_drops_a_glue_tool_that_shadows_an_allowed_or_toolbox_name(
    ...
    complete = FakeComplete(
        [json.dumps({"tools": ["date_parse"], "glue_tool": shadowing})]
    )
    result, _ = steps.select_tools(
        "goal", "domain", evaluator, ["date_parse"], complete, "strong"
    )
    assert result["glue_tool"] is None


def test_select_tools_drops_a_glue_tool_missing_required_fields(evaluator):
    incomplete = {"name": "x"}
    complete = FakeComplete(
...
def test_apply_playbook_returns_revised_prompt_and_applied_ids():
    lessons = [{"id": "l1", "domain_tags": ["x"]}, {"id": "l2", "domain_tags": ["y"]}]
    complete = FakeComplete(
        [json.dumps({"prompt": "revised prompt text", "applied_lesson_ids": ["l1"]})]
    )
    result, _ = steps.apply_playbook(
        "goal", "domain", "original", lessons, complete, "strong"
    )
    assert result == {"prompt": "revised prompt text", "applied_lessons": ["l1"]}


def test_apply_playbook_filters_out_unknown_lesson_ids():
    lessons = [{"id": "l1", "domain_tags": []}]
    complete = FakeComplete(
        [
            json.dumps(
                {"prompt": "revised", "applied_lesson_ids": ["l1", "does-not-exist"]}
            )
        ]
    )
    result, _ = steps.apply_playbook(
...
def test_apply_playbook_keeps_the_original_prompt_when_the_model_returns_nothing_usable():
    complete = FakeComplete([json.dumps({"prompt": "", "applied_lesson_ids": ["l1"]})])
    result, _ = steps.apply_playbook(
        "goal", "domain", "original", [], complete, "strong"
    )
