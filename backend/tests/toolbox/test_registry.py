...
def test_write_agent_tool_emits_an_importable_re_export(tmp_path):
    ...
    assert "Generated by toolbox.registry.write_agent_tool" in source

    generated = _import_from_path("json_validate", path)
    # A literal copy, not the same object: the package copy is what W6's
    # tools lever edits, independent of the shared toolbox module.
    assert generated.TOOL == registry.TOOLBOX["json_validate"].TOOL
    assert generated.TOOL is not registry.TOOLBOX["json_validate"].TOOL
    assert generated.run is registry.TOOLBOX["json_validate"].run
    assert json.loads(generated.run({"text": "[]"}))["top_level_type"] == "array"


def test_written_tool_dict_is_a_structurally_parseable_literal(tmp_path):
    """A caller must be able to read TOOL without importing (executing) the file."""
    import ast

    path = registry.write_agent_tool("github_get_label_taxonomy", tmp_path / "tools")
    tree = ast.parse(path.read_text(encoding="utf-8"))
    assignment = next(
        node for node in tree.body if isinstance(node, ast.Assign) and node.targets[0].id == "TOOL"
    )
    parsed = ast.literal_eval(assignment.value)
    assert parsed == registry.TOOLBOX["github_get_label_taxonomy"].TOOL
