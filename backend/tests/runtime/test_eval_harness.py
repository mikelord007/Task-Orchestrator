...
def test_a_looping_agent_is_nudged_then_aborted(
    ...
    assert nudge["evidence"]["tool"] == "lookup_ticket"
    ...
    assert any(
        step["type"] == "nudge" and "identical arguments" in step["message"]
...
def test_case_timeout_is_recorded_as_a_budget_drift(
    ...
    assert {d["evidence"]["reason"] for d in drift} == {"case_timeout"}
    assert llm.calls == []
    assert all(o.failure_signature == "drift:budget" for o in summary.cases)


def test_step_limit_aborts_a_runaway_case(
    toy_package, evaluator_path, ledger, knobs, tmp_path
):
