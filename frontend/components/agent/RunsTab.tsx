...
import type { RunSummary, Split } from "@/lib/types";
import { num, shortTs, usd, ms } from "@/lib/format";
import PassStrip from "@/components/PassStrip";
import Stat from "@/components/Stat";
import { DriftBadge, Empty, Pill, Td, Th } from "@/components/ui";

/** Case results for one run, grouped by case, filtered by split and version. */
...
function RunBlock({
  ...
  const stable = run.cases.filter((c) => c.passed_by_repeat.every(Boolean)).length;
  ...
  return (
    <section>
      <header className="flex flex-wrap items-end justify-between gap-4 border-b border-line pb-2">
        <div className="flex flex-wrap items-baseline gap-3">
          ...
          <span className="text-[11px] text-fg-mute">
            repeats {run.repeats} · {run.cases.length} cases · {shortTs(run.finished_ts)}
          </span>
        </div>
        <div className="flex flex-wrap gap-6">
          ...
          {drifted > 0 ? <Stat label="drifted cases" value={String(drifted)} tone="drift" size="md" /> : null}
        </div>
      </header>
      ...
      <table className="w-full text-[12px]">
        <thead>
          <tr>
            <Th className="w-[13%]">case</Th>
            <Th className="w-[11%]">repeats</Th>
            ...
            <tr key={c.case_id}>
              <Td>
                <Link
                  ...
                  title="Compare v0 and the current version on this case"
                >
                  {c.case_id}
                </Link>
              </Td>
              <Td>
                <PassStrip passed={c.passed_by_repeat} />
              </Td>
              ...
              <Td
                className={`text-right tabular-nums ${c.tool_errors > 0 ? "text-fail" : "text-fg-mute"}`}
              >
                {c.tool_errors}
              </Td>
              <Td>
              ...
              <Td className="text-[11px]">
                {c.trace_url ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-fg-mute hover:text-train"
                  >
                    trace
                  </a>
                ) : null}
                {c.trace_url && c.transcript_path ? (
                  <span className="text-fg-mute"> </span>
                ) : null}
                {c.transcript_path ? (
